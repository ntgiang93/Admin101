...
{
    private readonly Alias<Menu> m = new Alias<Menu>("m");
    ...
    {
        var query = new Query(m.Table)
            .Where(m.Col(x => x.IsDeleted), false)
             .Where(m.Col(x => x.IsActive), true)
             .OrderBy(m.Col(x => x.ParentId), m.Col(x => x.DisplayOrder));
    ...
    public async Task<List<MenuDto>> GetMenusByPermissionsAsync(List<RolePermission> permissions)
    {
        ...
        var query = new Query(m.Table)
                    .Where(m.Col(x => x.IsDeleted), false)
                    .Where(m.Col(x => x.IsActive), true)
                    .Where(q => q.WhereNull(m.Col(x => x.Sysmodule))
                                 .OrWhereIn(m.Col(x => x.Sysmodule), activeModules))
                    .OrderBy(m.Col(x => x.ParentId), m.Col(x => x.DisplayOrder));
        var compiledQuery = _compiler.Compile(query);
