...
public class UserService : GenericService<User, string>, IUserService
{
        ...
        serviceProvider)
    {
        ...
        _domain = appSettings.AppDomain?.TrimEnd('/') ?? string.Empty;
    }

    public async Task<PaginatedResultDto<UserTableDto>> GetPaginationAsync(UserTableRequestDto request)
    {
        var cacheKey = CacheKeyHelper.Generate($"{_cachePrefix}GetPaginationAsync", request);
        return await CacheService.GetOrCreateAsync<PaginatedResultDto<UserTableDto>>(cacheKey, async () =>
        {
    ...
    public async Task<bool> ChangeEmailAsync(string newEmail, string password)
    {
        ...
        await CacheService.SetAsync(verificationId, verificationData, TimeSpan.FromMinutes(30));
    ...
    public async Task<bool> VerifyEmailChangeAsync(VerifyAccountDto dto)
    {
        ...
        CacheService.Remove(dto.VerificationId);
        return result;
    }
    ...
    {
        var cacheKey = CacheKeyHelper.Generate($"{_cachePrefix}GetUserPermissionsAsync", userId);
