...
public class FileService : GenericService<FileStorage, int>, IFileService
{
    ...
    {
        var cacheKey = CacheKeyHelper.Generate($"{_cachePrefix}GetFilesByReference", referenceId, referenceType);
        var cachedResult = await CacheService.GetOrCreateAsync(cacheKey, async () =>
        {
