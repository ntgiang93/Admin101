...
import { type SelectOptionType } from '@/types/base/SelectOption'
...
export const useGetSysModule = () => {
  ...
  const cachedData = queryClient.getQueryData<SelectOptionType[]>([
    'getSysModules',
  ])

  return useQuery<SelectOptionType[], Error>({
    queryKey: ['getSysModules'],
    queryFn: async () => {
      const response = await apiService.get<ApiResponse<SelectOptionType[]>>(
...
export const useGetByType = (type: string) => {
  ...
  const cachedData = queryClient.getQueryData<CategoryDto[]>([
    endpoint + '/type',
    type,
  ])
  return useQuery<CategoryDto[], Error>({
...
export const useSave = () => {
  ...
  return useMutation({
    mutationFn: async (body: CategoryDto) => {
      let response
      if (body.id < 1) {
        response = await apiService.post<ApiResponse<CategoryDto>>(
          `${endpoint}`,
          body,
        )
      } else {
        response = await apiService.put<ApiResponse<CategoryDto>>(
          `${endpoint}`,
          body,
        )
      }
      if (response.success) {
        ...
        queryClient.invalidateQueries({
          queryKey: [endpoint + '/type', body.type],
        })
      }
    },
  })
