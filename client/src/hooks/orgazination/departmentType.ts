...
export const useGetAll = () => {
  ...
  var cachedData = queryClient.getQueryData<DepartmentTypeDto[]>([
    endpoint,
    'getAll',
  ])
  return useQuery<DepartmentTypeDto[], Error>({
    ...
    queryFn: async () => {
      const response = await apiService.get<ApiResponse<DepartmentTypeDto[]>>(
        `${endpoint}`,
      )
  ...
  return useMutation({
    mutationFn: async (payload: SaveDepartmentTypeDto) => {
      if (!payload.id || payload.id < 1) {
        return apiService.post<ApiResponse<DepartmentTypeDto>>(
          `${endpoint}`,
          payload,
        )
      }
      return apiService.put<ApiResponse<DepartmentTypeDto>>(
        `${endpoint}`,
        payload,
      )
    },
