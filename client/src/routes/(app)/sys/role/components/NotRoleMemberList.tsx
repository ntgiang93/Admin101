...
import {SelectedIcon} from "@/components/ui/icon/SelectedIcon.tsx";
import {useTranslation} from "react-i18next";
import {HugeiconsIcon} from "@hugeicons/react";
import {UserAdd01Icon} from "@hugeicons/core-free-icons";

export interface NotRoleMemberListRef {
    refresh: () => void;
}

interface NotRoleMemberListProps {
    role?: RoleDto
    selectedUsers: Set<string>
    onSelectedChange: (id: string, isSelected: boolean) => void
}

const NotRoleMemberList = forwardRef<NotRoleMemberListRef, NotRoleMemberListProps>((props: NotRoleMemberListProps, ref)=> {
    ...
    return (
        <Card ref={cardRef} variant={'transparent'} className="col-span-3 p-0 h-full">
            <Card.Header ref={cardHeaderRef}>
                <Card.Title className="flex gap-1 items-center"><HugeiconsIcon icon={UserAdd01Icon} size={20}/> {t('not_in_role')}</Card.Title>
                <SearchInput
                    ...
                    >
                        {(!users || users.length == 0) && (
                            <div className="flex h-full justify-center items-center p-8">
                                <p className="text-center">{t('everyone_is_member')}</p>
                            </div>
                        )}
                        <div
                            style={{
                                    ...
                                    >
                                        {isLoaderRow
                                            ...
                                            : (<div className={clsx('flex items-center justify-between gap-4 p-2 my-1 w-full',
                                                    ...
                                                    `${isSelected ? 'bg-accent-soft' : 'has-[*:hover]:bg-default-hover hover:bg-default-hover'}`)}>
...
export default NotRoleMemberList;
